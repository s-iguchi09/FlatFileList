...
- **モード B（ポーリング）**: **GitHub MCP ツールが無い場合**（ローカル PC の Claude Code CLI など）。
  `gh` CLI（Windows / PowerShell）でPR操作し、`gh api` を **バックグラウンドでポーリング**して待つ。
...
- **B（ポーリング）**: レビュー依頼の **直前に基準時刻を記録**（`since=$(date -u +%Y-%m-%dT%H:%M:%SZ)`）
  してから投稿:
  ```powershell
  gh pr comment <n> --body "@coderabbitai review"     # 初回は full review
  ```
...
- **A（Webhook）**: `sleep` ポーリングはしない。レビュー/CI 結果は `<github-webhook-activity>`
  ...
  `mcp__github__pull_request_read`（`get_review_comments` / `get_status` / `get_check_runs`）で
  状況を確認して続行する（ユーザーには聞かない）。
- **B（ポーリング）**: **バックグラウンドの監視ループ**で待つ（前景 `sleep` は使わず、
  `run_in_background`＝デタッチ実行）。**レート消費を最小化するため条件付きリクエスト（ETag）を使う**
  — 変化が無ければ GitHub は `304 Not Modified` を返し、**304 はレート制限を消費しない**。
  `$since`（ステップ 4B で記録）より後の `coderabbitai[bot]` 新規レビューが現れたらループ終了→自動再開:
  ```bash
  token=$(gh auth token)
  url="https://api.github.com/repos/s-iguchi09/FlatFileList/pulls/<n>/reviews"
  tmp=$(mktemp -d); etag=""; deadline=$(( $(date +%s) + 600 ))   # 10分で打ち切り
  while [ "$(date +%s)" -lt "$deadline" ]; do
    code=$(curl -sS -o "$tmp/rev.json" -D "$tmp/hdr.txt" -w '%{http_code}' \
      -H "Authorization: Bearer $token" -H "Accept: application/vnd.github+json" \
      ${etag:+-H "If-None-Match: $etag"} "$url" || echo 000)
    if [ "$code" = "200" ]; then                 # 変化あり（＝レート消費1）
      etag=$(awk 'tolower($1)=="etag:"{print $2}' "$tmp/hdr.txt" | tr -d "\r")
      n=$(jq "[.[]|select(.user.login==\"coderabbitai[bot]\" and .submitted_at>\"$since\")]|length" "$tmp/rev.json")
      [ "${n:-0}" -gt 0 ] && break                # 新レビュー到着
    fi                                            # 304/000 は無変化 → レート消費なし
    sleep 60                                      # 変化が無い間は 304 が返る
  done
  ```
  - **待機中に叩くのは reviews エンドポイント1本だけ**。インラインコメントの取得は
    「新レビュー検知後に1回だけ」行い、待機ループ中は取得しない。
  - 間隔は 60 秒（CodeRabbit は数分かかるので十分）。無変化は 304 なので **実消費はほぼゼロ**
    （変化のあった時だけ 1 消費）。必要なら残量を `gh api rate_limit` で確認できる。

**指摘の収集**（モード共通。A は MCP、B は `gh api`）:
...
**タイムアウト / フォールバック**: 10 分待っても新レビューが付かない場合は、最新レビュー本文を
直接読んで状況を判断する（判定文字列 `Actionable comments posted:` や bot ログイン名
`coderabbitai[bot]` は将来変わり得るため、文字列一致だけに依存しない）。それでも判断できない時のみ
ユーザーへ状況を報告する。
